...
if (isset($_POST['buat'])) {
	...
	if (!$post_jumlah) {
		$_SESSION['alert'] = ['danger', 'Failed!', 'Masih ada form kosong.'];
	        ...
	        $_SESSION['alert'] = ['danger', 'Failed!', 'Minimal jumlah Rp 500'];
	} else {
	$tanggal = date('Y-m-d H:i:s');
	$kode = random(11);
	
	$insert_db = $db->query("INSERT INTO history_saldo VALUES('', '{$session}', 'Pengurangan Saldo', '{$post_jumlah}', 'Membuat voucher, ID :: ({$kode})', '{$tanggal}')");
	...
	$insert_db = $db->query("INSERT INTO voucher_deposit VALUES('', '{$kode}', '{$session}', '{$post_jumlah}', 'belom diredeem', 'Available', '{$tanggal}')");
			...
			exit(header("Location: ".base_url('/reseller/buat_voucher')));
			} else {
...
<section id="dashboard-ecommerce">
    <div class="row">
        <div class="col-md-12 col-12">
            <div class="card">
                 <div class="card-body">
                    <h4 class="m-t-0 text-uppercase header-title"><i class=""></i> BUAT VOUCHER</h4><hr>
                    <form method="POST">
                        ...
                        <div class="form-group"> 
                                    <div class="row">
                                <div class="col-md-12">
                                        <button type="submit" name="buat" class="pull-right btn btn-block btn--md btn-primary waves-effect w-md waves-light"><i class=""></i> Submit</button> 
                                        </div>
                                </div>
                           </div>
                    </form>
                        ...
                        <div class="tab-pane active" id="riwayat" aria-labelledby="credit-tab" role="tabpanel">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered datatable" id="Myriwayat" width="100%">
                                    ...
                                    <tbody>
                                    <?php
                            $check_data = $db->query("SELECT * FROM voucher_deposit WHERE username = '{$session}' ORDER BY id DESC");
