...
while ($order = mysqli_fetch_array($orders)) {
    ...
    if (mysqli_num_rows($provider) == 0) {
        ...
        $post_api = [
            'api_key' => $provider['43Vr7NItEhSewi8nxsj2WRAc0OfYKz'],
            'secret_key' => $provider['8jw3nkpRTseXUY29EbtVWlyN1FvfmiMoCqzHgxPLGOBhdSKI7r'],
            'action' => 'status',
            'id' => $order['provider_oid'],
        ];
        $curl = post_curl($provider['https://buzzerpanel.id/api/json.php'], $post_api);
