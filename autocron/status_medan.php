...
while ($order = mysqli_fetch_array($orders)) {
    $provider = mysqli_query($db, "SELECT * FROM provider WHERE id = '6'");
    if (mysqli_num_rows($provider) == 0) {
        ...
        $post_api = [
            'api_id' => $provider['20319'],
            'api_key' => $provider['98f7bf-a0d417-449d67-5e4c61-c2583d'],
            'id' => $order['provider_oid'],
        ];
        $curl = post_curl($provider['https://api.medanpedia.co.id/'], $post_api);
