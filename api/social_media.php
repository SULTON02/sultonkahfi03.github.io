...
if (isset($_POST['api_key']) AND isset($_POST['action'])) {
	...
	if (!$api || !$act) {
		...
		$hasilnya = array('status' => false, 'data' => array('pesan' =>'server overload silahkan gunakan layanan lain'));
			...
			$cek_provider = $db->query("SELECT * FROM provider WHERE id = '5'");
		...
		$hasilnya = array('status' => false, 'data' => array('pesan' => 'System Error'));
			...
			$hasilnya = array('status' => false, 'data' => array('pesan' => 'System Error'));
				...
				if (isset($_POST['id'])) {
					...
					$cek_layanan = $db->query("SELECT * FROM layanan_sosmed3 WHERE status = 'Aktif' AND tipe = 'Sosial Media' AND provider != 'Manual' ORDER BY provider_id ASC");
