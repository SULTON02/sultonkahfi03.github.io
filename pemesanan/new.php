...
if (isset($_POST['pesan'])) {
    ...
    $cek_prov = $db->query("SELECT * FROM provider WHERE code = 'GM'");
    ...
    if (!$post_target || !$post_layanan || !$post_kategori || !$post_pin) {
        ...
        if ($provider == "MANUAL") {
            ...
            $curl = post_curl($q['link'].'social-media', $post_api);
            $result = json_decode($curl, true);
            if (isset($result['result']) and $result['result'] == true) {
                $provider_oid = $result['data']['trxid'];
                $result_api = true;
            }
        } else {
            $_SESSION['alert'] = ['danger', 'Failed!', '' . $result['message'] . ''];
        }
        //print_r($curl);
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-12">
                            <select class="form-control" id="kategori" name="kategori">
                                ...
                                <?php
                                $cek_kategori = $db->query("SELECT * FROM kategori_layanan ORDER BY nama ASC");
...
<script type="text/javascript">
    $(document).ready(function() {
        $("#kategori").change(function() {
            ...
            $.ajax({
                url: '<?= base_url() ?>/ajax/layanan_sosmed.php',
        ...
        $("#layanan").change(function() {
            ...
            $.ajax({
                url: '<?= base_url() ?>/ajax/catatan_sosmed.php',
            ...
            $.ajax({
                url: '<?= base_url() ?>/ajax/rate_sosmed.php',
                data: 'layanan=' + layanan,
                type: 'POST',
                dataType: 'html',
                success: function(msg) {
    ...
    function filterCategory(category) {
        $.ajax({
            type: "POST",
            url: "<?= base_url() ?>/ajax/kategori_sosmed.php",
