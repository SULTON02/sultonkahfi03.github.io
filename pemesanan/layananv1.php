...
if (isset($_POST['pesan'])) {
    $post_kategori = anti_xss($_POST['kategori']);
    $post_layanan = anti_xss($_POST['layanan']);
    $post_jumlah = anti_xss($_POST['jumlah']);
    $post_target = anti_xss($_POST['target']);
    $post_pin = anti_xss($_POST['pin']);
    $post_comments = $_POST['comments'];

    $cek_layanan = $db->query("SELECT * FROM layanan_sosmed1 WHERE service_id = '{$post_layanan}' AND status = 'Aktif'");
    $data_layanan = mysqli_fetch_assoc($cek_layanan);

    if ($data_user['level'] === "Reseller") {
        $harga_total = $data_layanan['harga_api'];
    } elseif ($data_user['level'] == "Admin") {
        $harga_total = $data_layanan['harga_api'];
    } else {
        $harga_total = $data_layanan['harga'];
    }

    $status_refill = $data_layanan['refill'];
    $cek_pesanan = $db->query("SELECT * FROM pembelian_sosmed WHERE target = '{$post_target}' AND status = 'Pending'");
    $data_pesanan = mysqli_fetch_assoc($cek_pesanan);

    $cek_harga = $harga_total / 1000;
    $cek_profit = $data_layanan['profit'] / 1000;
    
    $exCC = urlencode($post_comments) <> '' ? urlencode($post_comments) : null;
    ...
    if (!empty($post_comments)) {
        ...
        $pp1k = $cek_profit * $post_jumlah;
    }
    $order_id = random_number(6);
    $tanggal = date('Y-m-d H:i:s');
    $provider = $data_layanan['provider'];
    $exTarget = urlencode($post_target);

    $cek_provider = $db->query("SELECT * FROM provider WHERE code = '{$provider}'");
    $data_provider = mysqli_fetch_assoc($cek_provider);

    $cek_prov = $db->query("SELECT * FROM provider WHERE id = '5'");
    $q = mysqli_fetch_assoc($cek_prov);
    ...
    if (!$post_target || !$post_layanan || !$post_kategori || !$post_pin) {
    ...
    } elseif (mysqli_num_rows($cek_pesanan) == 1) {
        ...
        if ($provider == "MANUAL") {
            ...
            $curl = post_curl($q['link'], $post_api);
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-12">
                            <select class="form-control" id="kategori" name="kategori">
                                ...
                                <?php
                              $cek_kategori = $db->query("SELECT * FROM kategori_layanan1 ORDER BY nama ASC"); 
                    ...
                    <div class="form-group">
                        <div class="col-md-12">
                            ...
                            <div class="input-group input-group-merge">
                                ...
                                <input type="password" class="form-control" name="pin" onkeyup="this.value=this.value.replace(/[^\d]+/g,'')" placeholder="" minlength="2" maxlength="2" data-validation-required-message="This pin field is required" required />
                            </div>
                        </div>
                    </div>
...
$(document).ready(function() {
	$("#kategori").change(function() {
		...
		$.ajax({
			url: '<?= base_url() ?>/ajax/layanan_sosmed1.php',
	...
	$("#layanan").change(function() {
		...
		$.ajax({
			url: '<?= base_url() ?>/ajax/catatan_sosmed1.php',
		...
		$.ajax({
			url: '<?= base_url() ?>/ajax/rate_sosmed1.php',
			data: 'layanan=' + layanan,
			type: 'POST',
			dataType: 'html',
			success: function(msg) {
...
function filterCategory(category) {
    $.ajax({
        type: "POST",
        url: "<?= base_url() ?>/ajax/kategori_sosmed1.php",
