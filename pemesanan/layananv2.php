...
if (isset($_POST['pesan'])) {
    ...
    $encoded_target = urldecode($post_target);
    ...
    if (!$post_target || !$post_layanan || !$post_kategori || !$post_pin) {
        ...
        if ($provider == "MANUAL") {
            ...
            $exCC = '';
            $post_api = [
                'key' => $q['43Vr7NItEhSewi8nxsj2WRAc0OfYKz'],
                'action' => 'add',
                'service' => $exService,
                'link' => $exData,
                'quantity' => $exQuantity,
            ...
            $curl = post_curl($q['https://buzzerpanel.id/api/json.php'], $post_api);
            $result = json_decode($curl, true);
            if (isset($result['order']) and $result['order'] == true) {
                $provider_oid = $result['order'];
                $result_api = true;
            }
        } else {
            $_SESSION['alert'] = ['danger', 'Failed!', '' . $result['order'] . ''];
        }
        //print_r($result);
...
<script type="text/javascript">
    $(document).ready(function() {
        $("#kategori").change(function() {
            ...
            $.ajax({
                url: '<?= base_url() ?>/ajax/layanan_sosmed11.php',
        ...
        $("#layanan").change(function() {
            ...
            $.ajax({
                url: '<?= base_url() ?>/ajax/catatan_sosmed11.php',
            ...
            $.ajax({
                url: '<?= base_url() ?>/ajax/rate_sosmed11.php',
                data: 'layanan=' + layanan,
                type: 'POST',
                dataType: 'html',
                success: function(msg) {
    ...
    function filterCategory(category) {
        $.ajax({
            type: "POST",
            url: "<?= base_url() ?>/ajax/kategori_sosmed11.php",
